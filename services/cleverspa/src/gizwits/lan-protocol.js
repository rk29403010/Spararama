...
export function encodeVariableLength(value) {
  if (!Number.isSafeInteger(value) || value < 0) throw new TypeError("length must be a non-negative safe integer");
  const bytes = [];
  do {
    let next = value & 0x7f;
    value >>= 7;
...
export function buildPacket(command, payload = Buffer.alloc(0), flag = 0) {
  ...
  return Buffer.concat([PREFIX, encodeVariableLength(bodyLength), Buffer.from([flag]), commandBytes, payload]);
}

export function parsePacket(packet) {
  if (packet.length < 8 || !packet.subarray(0, 4).equals(PREFIX)) throw new Error("invalid Gizwits packet prefix");
  ...
  return { flag: packet[decoded.nextOffset], command: packet.readUInt16BE(decoded.nextOffset + 1), payload: packet.subarray(decoded.nextOffset + 3) };
}
...
export function extractPackets(input) {
  ...
  while (buffer.length >= 5) {
    if (!buffer.subarray(0, 4).equals(PREFIX)) throw new Error("invalid Gizwits stream prefix");
...
export function parseDiscoveryPacket(packet, ip = "") {
  ...
  if (parsed.command !== 0x0004) throw new Error(`unexpected discovery command 0x${parsed.command.toString(16)}`);
  let offset = 0;
  const uid = readLengthField(parsed.payload, offset); offset = uid.nextOffset;
  const mac = readLengthField(parsed.payload, offset); offset = mac.nextOffset;
  const firmware = readLengthField(parsed.payload, offset); offset = firmware.nextOffset;
  const productKey = readLengthField(parsed.payload, offset);
  return { ip, uid: uid.value.toString("ascii"), mac: [...mac.value].map((byte) => byte.toString(16).padStart(2, "0")).join(":"), firmwareVersion: firmware.value.toString("ascii"), productKey: productKey.value.toString("ascii") };
}

function decodeNumber(attribute, bytes) {
  const raw = attribute.dataType === "uint16" ? bytes.readUInt16BE(attribute.byteOffset) : bytes[attribute.byteOffset];
  return raw * (attribute.ratio ?? 1) + (attribute.addition ?? 0);
}

export function decodeSpaStatus(bytes) {
  if (bytes.length < CLEVERSPA_STATUS_LENGTH) throw new Error(`CleverSpa status requires ${CLEVERSPA_STATUS_LENGTH} bytes`);
  const state = {};
  for (const attribute of CLEVERSPA_MODEL.attributes) {
    state[attribute.name] = attribute.dataType === "bool"
      ? ((bytes[attribute.byteOffset] >> attribute.bitOffset) & 1) === 1
      : decodeNumber(attribute, bytes);
  }
  return state;
}

export function parseStatusPayload(payload) {
  if (payload.length < CLEVERSPA_STATUS_LENGTH + 1) throw new Error("truncated CleverSpa status payload");
  const actionOffset = payload.length - CLEVERSPA_STATUS_LENGTH - 1;
  const action = payload[actionOffset];
  if (action !== 0x03 && action !== 0x04) throw new Error(`unexpected CleverSpa status action 0x${action.toString(16)}`);
...
function encodeWireNumber(attribute, value) {
  ...
  if (value < attribute.minimum || value > attribute.maximum) throw new RangeError(`${attribute.name} must be between ${attribute.minimum} and ${attribute.maximum}`);
...
export function buildAttributeUpdate(updates, sequence = Math.floor(Date.now() / 1000) & 0xffff) {
  const writable = CLEVERSPA_MODEL.attributes.filter((attribute) => attribute.type === "status_writable");
  ...
  const valuesLength = Math.max(...writable.map((attribute) => attribute.byteOffset + (attribute.unit === "byte" ? attribute.length : 1)));
